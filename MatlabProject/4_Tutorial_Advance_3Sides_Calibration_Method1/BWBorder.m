...
function BWEdge = BWBorder(Image)

% detect region of black pixels
Black = Image(:,:,1)<=70 & Image(:,:,2)<=70 & Image(:,:,3)<=70; 
% detect region of white pixels
White = Image(:,:,1)>=245 & Image(:,:,2)>=245 & Image(:,:,3)>=245; 
...
BWEdge=ismember(labelmatrix(bwconncomp(BWEdge,8)), find([stats.Area] > 200));
